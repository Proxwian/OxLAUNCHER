...
const main = async () => {
  ...
  try {
    const { data: releasesList } = await axios.default.get(
      `https://api.github.com/repos/Proxwian/OxLAUNCHER/releases`,
      {
        headers: {
      ...
      for (const element in rawChangeLog) {
        if (rawChangeLog[element].length) {
          ...
          for (const e of rawChangeLog[element]) {
            ...
            const advanced =
              e?.advanced?.cm &&
              ` ([${e?.advanced?.cm}](https://github.com/Proxwian/OxLAUNCHER/commit/${e?.advanced?.cm})` +
                `${
                  prSplit
                    ? ` | [#${e?.advanced.pr}](https://github.com/Proxwian/OxLAUNCHER/pull/${prSplit[0]}` +
